...
FuncDef::FuncDef(string name, vector<TypeDef> tds, bool payable)
{
    ...
    this->selectorBytes = functionSelector(this->name /* name */, this->tds /* type defs */);
    this->selector = (static_cast<uint32_t>(this->selectorBytes[0]) << 24) +
                     (static_cast<uint32_t>(this->selectorBytes[1]) << 16) +
                     (static_cast<uint32_t>(this->selectorBytes[2]) << 8) +
                     static_cast<uint32_t>(this->selectorBytes[3]);
}
...
int ContractABI::ithFuncDefIndex(int i)
{
    if (constructorIdx < 0 || i < constructorIdx)
    {
        return i;
    }
    else
...
ContractABI::ContractABI(string abiJson)
{
    ...
    for (auto node : root)
    {
        ...
        if (type == "fallback")
        {
            ...
            auto fd = FuncDef("fallback", tds, payable);
        ...
        { 
            ...
            auto fd = FuncDef(name, tds, payable);
            this->fds.push_back(fd);
            if (name == "")
            {
                constructorIdx = idx;
    ...
    totalFuncsNum = idx - (constructorIdx < 0 ? 0 : 1   );
}
...
vector<bytes> ContractABI::encodeFunctions()
{
    ...
    for (auto fd : fds)
    {
        if (fd.name != "")
        {
            bytes selector = bytes(fd.selectorBytes.begin(), fd.selectorBytes.end());
...
vector<bytes> ContractABI::encodeFunctions(bytes data, vector<size_t> order)
{
    vector<bytes> ret;
    vector<FuncDef> curFds(order.size()); 
    auto it = find_if(fds.begin(), fds.end(), [](FuncDef fd) { return fd.name == ""; });
    for (int i = 0; i < order.size(); i++)
    {
        curFds[i] = fds[order[i]];
    }
    if (it != fds.end())
    {
        curFds.insert(curFds.begin(), *it);
    }
    else
    {
        ret.push_back(bytes());
...
// vector<bytes> ContractABI::encodeProperties() {
//     vector<bytes> ret;
//     for (auto fd : echidna_properties)
//     {
//         if (fd.name != "")
//         {
//             bytes selector = bytes(fd.selectorBytes.begin(), fd.selectorBytes.end());
...
bytes functionSelector(string name, vector<TypeDef> tds)
{
